...
def CrearDominio():
    ...
    if prueba is False:
        dominio = (pedirValor("\033[95mCual es el nombre de tu dominio (ej. Jacobo.castelao):\033[0m\n"))
        lista = dominio.split(".")
...
def CrearContra():
    ...
    if prueba is False:
        contraseña = (pedirValor("\033[95mCual la contraseña de tu dominio (ej. 12345):\033[0m\n"))
        f = open("contra.txt", "a")
        f.write(contraseña)
        f.close()
...
def CrearOu():
    ...
    if comprueba_espacios_puntos(unidad_o):
        ...
        cargaOU= ("ldapadd -f crear_ou.ldif -D cn=" + str(administrador) + ",dc=" + str(lista[0])
                  + ",dc=" + str(lista[1]) + " -w " + str(contraseña))
        os.system (cargaOU)
...
while not isSalir:
    ...
    #Creamos los fichero en caso de que no existan
    creacionficheros()
    CrearDominio()
    CrearAdmin()
    CrearContra()
